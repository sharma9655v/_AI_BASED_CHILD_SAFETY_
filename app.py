...
def send_alert_thread(contact, msg_body, speech, lang_code):
    """
    Sends alerts INDEPENDENTLY. If one fails, the others still go through.
    """
    ...
    try:
        ...
        print(f"⚠️ WhatsApp failed for {contact} (Check Sandbox/Join): {e}")
    ...
    try:
        ...
        print(f"⚠️ SMS failed for {contact} (Check Geo-Permissions): {e}")
    ...
    try:
        ...
        print(f"⚠️ Call failed for {contact}: {e}")
...
def trigger_sos(lat, lon, language_choice):
    ...
    for contact in EMERGENCY_CONTACTS:
        t = threading.Thread(target=send_alert_thread, args=(contact, msg_body, speech, lang_code))
        threads.append(t)
        t.start()
...
with tab3:
    ...
    st.warning("⚠️ Ensure WhatsApp Sandbox is joined! Text 'join <keyword>' to +15075195618")
    ...
    if st.button("🆘 ACTIVATE SOS"):
        if location.get("latitude"):
            with st.spinner("Dispatching Alerts..."):
                trigger_sos(location["latitude"], location["longitude"], sos_lang)
                time.sleep(2) 
            st.success("✅ Alerts Dispatched! Check Phones.")
        else:
